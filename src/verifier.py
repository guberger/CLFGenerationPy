...
class VerifierSimple:
    ...
    def check_expr(self, expr):
        ...
        z3syms, syms_map = \
            z3utils.create_z3syms_from_spsyms(ctx, self.spsyms)
        ...
        z3expr = z3utils.convert_spexpr_to_z3expr(syms_map, expr)
        solver.add(z3expr <= 0)

        res = solver.check()
        if res == z3.sat:
            model = solver.model()
            vars_ = z3utils.get_vars_from_z3model(syms_map, model)
...
class VerifierParam:
    ...
    def check_expr(self, expr):
        ...
        z3syms_param, syms_param_map = \
            z3utils.create_z3syms_from_spsyms(ctx, self.spsyms_param)
        ...
        z3expr = z3utils.convert_spexpr_to_z3expr(syms_all_map, expr)
